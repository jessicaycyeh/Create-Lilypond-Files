...
def find_type(d, bpm): #input:duration [s], output: '16' or '8' ...etc
...
def get_template_t(tot_beats):
	template_t = np.zeros(tot_beats*4)
	for i in range(tot_beats):
		template_t[4*i] = i + 0. 
		template_t[4*i+1] = i + 0.25
		template_t[4*i+2] = i + 0.5
		template_t[4*i+3] = i + 0.75
	return template_t

def find_beat_id(t, bpm, template_t):
	one_beat_duration = 60 / bpm 
	delta_t = np.abs(t - template_t * one_beat_duration)
	imin = np.argmin(delta_t)
	min_delta = delta_t[imin]
	identified_beat = template_t[imin]
	return identified_beat 
